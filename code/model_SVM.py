...
def model_SVM(nrow=10000,NUM_OF_TIME_STAMP_FOR_DERIV=30, NUM_OF_TIME_STAMP_FOR_RESPONSE=5):
	...
	#extract the design matrix, i.e. get rid of the last column. X.shape[1]=126 
	X = sample_data.iloc[:,:X.shape[1]]
	#extract the response for the sample data, i.e. get the last column 
	y = sample_data.iloc[:,X.shape[1]].values
	...
	testing_data = split_modeling_data(NUM_OF_TIME_STAMP_FOR_DERIV, 
		NUM_OF_TIME_STAMP_FOR_RESPONSE)[1].iloc[:,:X.shape[1]]
